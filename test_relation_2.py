...
for i in range(0,len(test_relation_img)):
    ...
    B=np.zeros(600)
    Br=np.zeros(600)
    B[0:300]=obj2vec[test_entity0_index[i]]
    B[300:600]=obj2vec[test_entity1_index[i]]
    Br[0:300] = obj2vec[test_entity1_index[i]]
    Br[300:600] = obj2vec[test_entity0_index[i]]
    ...
    R_t=BR-B.dot(R_X)
    R_r=BR-Br.dot(R_X)
    #if R_t.dot(R[test_relation_index[i]])<R_r.dot(R[test_relation_index[i]]):
    #    continue

    t = 0
    for j in range(1, 70):
        #if R_t.dot(R[j]) > R_t.dot(R[test_relation_index[i]]):
        if np.linalg.norm(R_t-R[j])<np.linalg.norm(R_t-R[test_relation_index[i]]):
            t=t+1
    #print R_t.dot(R[test_relation_index[i]])/np.linalg.norm(R_t)
