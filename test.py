...
for i in range(0,1000):
    ...
    B0=get_feature(image,net,transformer,X,train_bbox0[i][0],train_bbox0[i][1],train_bbox0[i][2],train_bbox0[i][3])

    #The second entity
    B1= get_feature(image, net, transformer, X,train_bbox1[i][0], train_bbox1[i][1], train_bbox1[i][2],
                          train_bbox1[i][3])
    ...
    t = 0
    for j in range(1, 70):
        if R_t.dot(R[j]) > R_t.dot(R[t]):
            t = j

    if t == train_relation_index[i]:
        sum = sum + 1
